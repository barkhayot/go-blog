...
func Signup(c *gin.Context) {
	...
	// Record data
	user := model.User{Email: body.Email, Password: string(hash), Username: body.Username, FirstName: body.FirsName, LastName: body.LastName}
...
func Logout(c *gin.Context) {
	// Get the user's JWT token from the request headers or cookies
	tokenString, err := c.Request.Cookie("Autorization") //c.Request.Cookie("jwt_token")
	fmt.Println(tokenString)
	if err != nil {
		c.JSON(400, gin.H{"message": "Token not found"})
		return
	}
	// Delete the JWT token cookie on the client side
